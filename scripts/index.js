...
В основном правильно обрабатываются ответы от сервера с помощью методов имеющихся классов.
Предусмотрено обнаружение ошибок и вывод их в консоль.


Что можно улучшить.

1. Сделать метод close класса popup независимым от события event (подробные комментарии в классе Popup). --- OK!

 Что надо исправить.
1. Перенести обработчик события 'submit' формы профиля и его добавление к форме из класса UserInfo в класс PopupEdit
(подробные комментарии в классе UserInfo). --- OK!

2. Не вызывать метод setUserInfo до прихода ответа от сервера (подробные комментарии в классе UserInfo). --- OK!

-------------------- Спасибо! Возможно ли прислать работу на проверку после выполнения доп. заданий? --------------


REVIEW2  по заданию 9.

Теперь обработка сабмита формы профиля происходит там, где надо.

Но, надо исправить.

1. Структуру метода updateUserInfo (читайте об этом в модуле класса UserInfo, затем в модуле класса PopupEdit,
  затем в этом модуле около задания экземпляра класса UserInfo.)
